...
A common theme throughout our previous analysis has been treating Limit Continuous Poker as a generalization between Continuous Poker and NLCP. The value of the games follows this theme nicely.

To see this visually, we can look at the expected value of the game as a function of the limits $L$ and $U$ in Figure \ref{fig:payoffs_vs_LU}. This is calculated by integrating the payoff in Nash equilibrium over the joint distribution of hand strengths $x$ and $y$ (the square $[0, 1] \times [0, 1]$). 

As expected, the game value increases for more lenient limits (top left of Figure \ref{fig:payoffs_vs_LU}) and generally decreases for more strict limits (moving down/right). This also aligns with the known result that a fixed pot-size bet of $B=1$ maximizes the expected value for the bettor in Continuous Poker, as seen by the fact that $(1, 1)$ is the highest value on the diagonal. Interestingly, if we zoom out (Figure ), we see that the game value is highly affected by 

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{value_by_LU.png}
    \caption{Game value of Limit Continuous Poker as a function of the limits $L$ and $U$.}
    \label{fig:payoffs_vs_LU}
\end{figure}

This can be broken down further by consider the combinations of hand strengths which result in certain payoffs in Nash equilibrium, since clearly not all types are equally favorable for each player. Figure \ref{fig:payoffs} shows the payoffs as a function of hand strengths $x$ and $y$ for various values of $L, U$. Some notable features of these plots are:
