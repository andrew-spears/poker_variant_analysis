...
for the bettor\todo{cite}. \todo{restate LCP game value from earlier}

The expected value of NLCP is 

$$ V = \frac{1}{14} $$

for the bettor\todo{citation-ferguson}\footnote{Would be 1/7 for an ante of 1, but the value is halved with an ante of 0.5}. 


If we consider the combinations of hand strengths which result in certain payoffs in Nash equilibrium, we can see visually how the game value shifts from Continuous Poker to NLCP (see Figure \ref{fig:payoffs}). Some notable features of the figure are:
